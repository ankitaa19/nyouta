...
const Login = () => {
    ...
    try {
      if (!requiresOtp) {
        ...
        if (response.ok) {
          if (data.requiresOtp) {
            setRequiresOtp(true);
            setWelcomeMessage(`Welcome ${data.user.name}, please enter the OTP sent to your email.`);
          } else {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ textAlign: 'center', marginBottom: '2rem', fontWeight: '700', letterSpacing: '1px' }}>Login</h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        >
          {loading ? (requiresOtp ? 'Verifying OTP...' : 'Logging in...') : (requiresOtp ? 'Verify OTP' : 'Login')}
        </button>
      </form>
    </div>
  );
